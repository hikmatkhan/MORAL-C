...
from datasets.ocelot import OcelotDataset

cuda_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


def get_dataloader(root_dir, batch_size=64, shuffle=False, num_workers=4, organ =None):
    

    dataset = OcelotDataset(
        root_dir=root_dir,
        organ= organ #"head-and-neck"
    )
...
def run_eval(checkpoint_path, image_folder, x):
    ...
    dataloader = get_dataloader(image_folder, organ = x)
...
def main(ckpts_root_path: Path, image_folder: str):
    ...
    organlist = ["bladder","endometrium", "head-and-neck", "kidney","prostate", "stomach"]
    for x in organlist:
        print("---------------------------------------------------------------------------------------------")
        print(f'Running Evaluation for organ', x)
        for model_path in models:
            run_eval(model_path, image_folder, x)
            print("-----------------------------------------------------------------------------------------")
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--image_folder', default='./preprocess/BCSS/BCSS_Data/patches/', type=str, help='Path to the folder containing images.')
    parser.add_argument('--image_folder', default='F:/OCELOT/final_data/', type=str, help='Path to the folder containing images.')
